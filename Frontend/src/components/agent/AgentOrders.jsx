...
import { Package, Loader, ExternalLink, Pencil, Plus, MapPin } from 'lucide-react';
...
const TrackingCell = ({ order, onEdit, onTrack }) => {
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button
          ...
          className="text-accent hover:text-red-700 transition-colors p-0.5"
          title="Track package"
        >
        ...
        <button
          onClick={() => onEdit(order)}
          className="text-gray-400 dark:text-zinc-500 hover:text-accent transition-colors p-0.5"
                ...
                {orders.map((order, idx) => (
                  <tr key={order.id}
                    ...
                    <td className="px-5 py-4">
                      <TrackingCell
                        order={order}
                        onEdit={setSelectedOrderForTracking}
                        onTrack={setTrackingModalOrder}
                      />
                    </td>
                  </tr>
                ))}
